...
import { ConnectionState, CompanyConfig } from '../types';
...
export const AssistantPage: React.FC = () => {
  ...
  const { connectionState, connect, disconnect, sendTextMessage, volume, transcriptions, error } = useGeminiLive(config!);
  ...
  useEffect(() => {
    if (transcriptions.length > 0) {
      const saveConversation = async () => {
        try {
          const baseUrl = 'https://ai.marketbizz.in/api/conversations';
          await fetch(baseUrl, {
            method: 'POST',
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-900 to-slate-800 text-white overflow-hidden flex flex-col font-inter">
      ...
      <header className="px-8 py-6 border-b border-slate-700/50 bg-slate-900/50 backdrop-blur-sm flex justify-between items-center z-10 sticky top-0 shrink-0">
        ...
        <div className="flex items-center gap-6">
          <div className={`px-3 py-1 rounded-full text-xs font-medium flex items-center gap-2 transition-colors ${
               ...
               {!isConnected ? (
                  <div className="flex justify-center">
                      <button 
                        ...
                        className={`group relative flex items-center gap-4 px-10 py-4 rounded-full text-white font-semibold shadow-xl transition-all duration-300 hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed ${
                            isError 
